...
### 3. 底栏「上次粘贴」定位按钮
- 底部工具栏在组名与「全部粘贴」之间新增**「上次粘贴」文字胶囊按钮**（小对勾 + 文字），底色 / 文字 / 描边沿用底栏既有的 glass pill token，与组名 chip 同一套视觉语言，亮暗模式自动跟随。
- 点击后**全程留在圆盘内**：静默切到目标页 + 目标组 → 对应扇区高亮 → 预览窗同步。圆盘不关闭、主窗口不抢焦点。
- 鼠标重新划入圆盘或点击任意扇区即释放程序化聚焦，回归正常 hover。
- 尚未粘贴过任何槽位时整颗置灰并禁用，tooltip 提示「尚未粘贴过任何槽位」。
...
- 空态收起：没悬停任何槽位时，预览窗收成一条标题栏 —— 内容区与整幅磨砂底走 `if` 条件渲染（不是 `.opacity(0)` / `.hidden()` 留占位），圆盘右侧不再挂着一块空磨砂色块；标题栏（标题 / 缩放 / 置顶）始终常驻可点，置顶后圆盘关闭也能随时取消置顶。
- 收放不再跳动：标题栏严格顶部锚定，内容区只向下伸展，展开 / 收起走 0.15s `easeOut` 过渡。
- 面板高度封顶为屏幕可用高度的 60%（常规屏仍是 480pt，小屏 / 分屏自动收缩），避免底部内容被顶到屏幕外。
...
```
8018c38d8c9c5fed8973ad627204328ba9b8086baaf0cc303f0d8eb3ded32c5e  ClipSlots_v2.11.3.dmg
```
