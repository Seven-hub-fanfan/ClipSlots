...
enum AppTheme {
    ...
    ///
    /// 现在浅色模式给一档**比卡片略沉的面**，边界感交给 `canvasTextNodeStroke` 的描边来做；
    /// 深色模式保持原来那档（比卡片底 0.105 高一档，不与卡片糊在一起）。
    /// "一眼分辨节点类型"这件事由"整块内容区带描边的底 + 描边"继续承担，不再依赖反相的黑。
    ///
    /// ★ v2.11.17 修（用户反馈"这个边像没扣干净"）：v2.11.16 浅色给的是**不透明冷灰**
    /// `rgb(245,246,248)`——B 比 R 高。而这套皮肤的卡片底是**暖奶白** `rgb(253,253,250)`（B 比 R 低），
    /// 图片节点的预览面用的是 `previewBackground` = **半透明黑 3.5%**，压在暖底上出来仍然是暖的
    /// `rgb(243,243,240)`。于是整张画布只有文本节点这一块是冷调，它和周围暖白的交界处出现一条
    /// 冷暖分界——观感正是"贴上去的一块、边没抠干净"。
    ///
    /// 所以浅色改成**和图片节点预览面同一档半透明黑**：透明面天生继承卡片的色温（卡片改暖改冷它
