...
struct CanvasNodeCardView: View {
    ...
    /// 正文文本块（**不是**正文区盒子）的 1x 高度。
    private var promptTextBoxHeight1x: CGFloat {
    ...
    private var promptLineLimit: Int {
        ...
        let fits = Int((promptTextBoxHeight1x / lineHeight).rounded(.down))
    ...
    /// ★ v2.11.12：渐隐比例按 1x 算 → 是个**常数**，遮罩边界不再随缩放上下爬。
    private var promptFadeMask: LinearGradient {
        let fade = min(0.28, 10 / promptTextBoxHeight1x)
