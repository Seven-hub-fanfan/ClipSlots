...
///
/// 用户要的效果：槽位的多条内容在节点里以一叠卡片呈现，鼠标悬停时展开。
///
/// ## ★ v2.11.8 九轮：展开风格收敛为**唯一一种**（扇形）
///
/// 八轮曾并存三种风格（扇形 / 水平轮播 / 交替叠放）+ 一个右上角切换按钮 + 一个右键子菜单。
/// 用户九轮明确要求删掉后两种，只留扇形，且"不再需要用户切换"。所以本文件里只剩 `layouts`
/// 一套扇形数学；`ExpandStyle` 作为**落盘字段的类型**保留（见那边的注释：老 JSON 里存着
/// `carousel` / `stackedScatter` 字符串，枚举直接删项会让解码抛错、整张画布读不出来）。
///
...
public enum CanvasFanGeometry {
    ...
    ///
    /// ## ★ v2.11.8 九轮：只剩一种（`.fanOut`），但类型必须留着
    ///
    /// 用户要求删掉「水平轮播」「交替叠放」，扇形成为唯一样式、不再提供切换入口。于是这个枚举
    /// **只剩一个 case**，看起来像是可以整个删掉、把字段也从 `CanvasNode` 里抹掉 —— 不行：
    ///
    /// 八轮（`v2.11.8` 已发布并装机）会把 `"carousel"` / `"stackedScatter"` 真的写进画布 JSON。
    /// `RawRepresentable` 的合成 `init(from:)` 遇到不认识的 `rawValue` 是**抛错**，而不是返回 nil；
    /// `decodeIfPresent` 也拦不住（它只处理"键不存在 / null"，值在但不认识照样抛）。抛错会顺着
    /// `CanvasNode` → `CanvasDocument` 冒到最外层，症状是**整张画布读不出来**、用户所有节点凭空消失。
    ///
    /// 所以这里手写 `init(from:)` 做**向前兼容降级**：任何不认识的字符串都迁移成 `.fanOut`。
    /// 这也顺手覆盖了未来再删风格的情况。
    public enum ExpandStyle: String, Codable, CaseIterable, Equatable {
        case fanOut

        /// 老数据里的废弃值（八轮的 `carousel` / `stackedScatter`，以及任何手改坏的字符串）
        /// 一律迁移到扇形，绝不抛错。
        public init(from decoder: Decoder) throws {
            let raw = try decoder.singleValueContainer().decode(String.self)
            self = ExpandStyle(rawValue: raw) ?? .fanOut
        }
