...
public enum CanvasZoomLayout {
    ...
    /// ★★ v2.11.13：**恒为 1**（反向补偿彻底退休）。
    ///
    /// 反向补偿存在的唯一理由是"让文字的屏幕尺寸不随缩放变"。而屏幕字号恒定 + 盒子宽度随
    /// 缩放变 = 每行字数必变 = 重排，这是三轮/八轮/九轮/v2.11.11/v2.11.12 五次返工都没能绕开的
    /// 数学事实（用户第 6 次打回：“太奇怪了实在不行就不要矢量好吗？”）。v2.11.13 接受用户给的
    /// 出路：**不要矢量重排** —— 卡片按固定基准排版一次，缩放只做渲染期变换，文字跟着等比放大
    /// 缩小（像放大一张图），代价是极端放大时字形边缘会软，换来的是"缩放时文字绝对不自己动"。
    ///
    /// 函数保留（调用点还在，签名也被 smoke 盯着），但返回常量 1：任何非 1 的补偿都会让文字与
    /// 卡片脱钩，而脱钩就一定重新引入重排。
    ...
    /// ★★ v2.11.13：**固定排版基准**。所有节点卡片的内部排版永远按这个尺度算一次。
    ///
    /// 取 2 而不是 1 的原因是**超采样**：排版尺寸放大 2 倍（字号、padding、线宽同乘 2，比例与
    /// 1x 严格全等，所以换行位置与 1x 完全相同），再由节点层 `scaleEffect(zoom / 2)` 缩回去。
    /// 于是 100%~200% 这段最常用的区间里，文字的光栅化分辨率还是够的，不会一放大就发虚；
    /// 而由于基准是常量，`zoom` 怎么变都**不触发重新排版** —— 这正是本轮要的性质。
    ///
    /// 想让极端放大（>300%）也锐利，把这个值调大即可（代价是内存/绘制开销按平方涨）。
    public static let layoutBaseScale: CGFloat = 2
