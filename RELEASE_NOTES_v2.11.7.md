...
- **简洁模式左上角 Logo 改为展示 App 真实图标**（白底 squircle + 黑色三叠卵石），**44pt**，无底板 / 无描边 / 无投影，与右侧标题垂直居中。此前给它套了中性灰底板，在浅色简洁界面上变成一个「大白圆角按钮」，看着像可点的控件，而它只是个 logo。多彩模式的品牌渐变底板 + 叠层图形保持原样，一点没动。
  ...
  - 尺寸从 30pt 提到 44pt：`frame` 给的是图标**画布**边长，不是可见图案边长。量过 `assets/AppIcon.png`（1024 画布）——白色 squircle + 投影的 alpha 包围盒只有 886px（86.5%），黑色石头图案更小，只有 474px（46%）。30pt 画布下石头实际只有 ~14pt，挨着 22pt bold 的标题就显得单薄；44pt 下 squircle ≈38pt、石头 ≈20pt，与标题文字体量齐平，且正好等于多彩模式底板宽度，切皮肤时左侧内容不会横向跳动。
...
```
7771493f685d1f7a996f8f54bd4e180a4dbb4e495d921ea12e54c93c4546eeb5
```
