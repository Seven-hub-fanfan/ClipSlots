...
requires: macOS + 已安装 ClipSlots v2.9.1+（CLI 位于 `~/bin/clipslots`）
---
...
## 1. 命令参考（v2.9.1，共 13 个）

### 只读
```bash
clipslots version                                  # {"ok":true,"version":"2.9.1"}
...
clipslots list [--group <id>]                      # 某组 1..10 槽位摘要 [{slot,label,preview,type,empty}]
clipslots read <slot> [--group <id>]               # 单槽完整内容 {slot,label,preview,text,htmlSource,types,attachmentCount,empty}
...
2. **以 `ok` 判断成败**，`ok:false` 读 `error`；不要只看退出码文案。
3. **槽位范围** 1..10，越界返回 `ok:false`。
4. **主体 vs 附件**：`write` 改主体（保留附件）；`write-attachment` 只加附件（不动主体）；二者配合实现模式A/B/C。
5. **paste 语义**：只送入剪贴板，不自动粘贴；需要真正粘贴时提示用户 Cmd+V。
6. **多组/多页**：涉及非默认组先 `groups`/`pages` 拿真实 id，不要臆测。
7. **容量与命名**：严格按第 3、4 节；溢出用 `-2/-3` 或新页面，命名遵守字数上限与阿拉伯数字序号。
8. **富文本**：`read` 的 `htmlSource` 非空表示有 HTML 源；CLI `write` 只写纯文本，需保 HTML 走 GUI。
